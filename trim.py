...
def trim_xray_images(img1, img2, image_size, label):
    ...
    img1 = cv2.copyMakeBorder(img1, abs(min(0, y1)), 0, abs(min(0, x1)), 0, borderType=cv2.BORDER_CONSTANT, value=[255,255,255])
    x1 = max(x1, 0)
    y1 = max(y1, 0)
    img2 = cv2.copyMakeBorder(img2, abs(min(0, y2)), 0, abs(min(0, x2)), 0, borderType=cv2.BORDER_CONSTANT, value=[255,255,255])
    x2 = max(x2, 0)
    y2 = max(y2, 0)
